...
class _CurrentSongScreenState extends State<CurrentSongScreen> {
  ...
  Future<void> _setupAudioPlayer() async {
    try {
      await widget.audioPlayer.setSource(
        AssetSource(widget.song.audioVersions[currentLanguage]!),
      );
  ...
  void switchLanguage(String language) async {
    ...
    try {
      ...
      await widget.audioPlayer.setSource(
        AssetSource(widget.song.audioVersions[language]!),
      );
      if (isPlaying) {
        await widget.audioPlayer.resume();
      }
  ...
  void _toggleRepeat() async {
    ...
    await widget.audioPlayer.setReleaseMode(
      isRepeatOn ? ReleaseMode.loop : ReleaseMode.release,
    );
  }

  void _showLyrics() async {
    final lyrics = await rootBundle.loadString(
      'assets/${widget.song.lyricsVersions[currentLanguage]!}',
    );
          ...
          Container(
            ...
            child: Container(
              ...
              child: Row(
                ...
                children:
                    widget.song.audioVersions.keys.map((lang) {
            ...
            TextButton(
              ...
              child: Row(
                ...
                children: [
                  ...
                  const Text('Lyrics', style: TextStyle(color: Colors.black87)),
                ],
              ),
            ),
            const SizedBox(height: 32),
